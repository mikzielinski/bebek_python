...
def draw_pitch(pitch_config_dict,cell_definition,Master_frame,position_dict,char_dict_pic):
    ...
    for j in range(pitch_config_dict['size_l']):
        Label(frame, text =j,width=cell_definition['width'],height=cell_definition['lenght'],image=cell_definition['image'],compound='center').grid(row=j,column=0)
        j+1
        if j == pitch_config_dict['size_l']:
            break
    #draw and set rows
    for index, i in enumerate(alc):   
        Label(frame, text =i,width=cell_definition['width'], height=cell_definition['lenght'],image=cell_definition['image'],compound='center').grid(row=0,column=index+1)
    ...
    for r in range(index+1):
        ...
        c=1
        for c in range(j):
               dict_obj= Tools_class.bebek_tools.create_cell(c+1,r+1,frame,"empty",pitch_config_dict,board_position_dict,cell_definition)
    ...
    d =Tools_class.bebek_tools.set_char(4,6,frame,orc,pitch_config_dict,board_position_dict,char_dict_pic)
    board_position_dict["4:6"]= d
    
    dwarf = Team_class.bebek_character.dwarf_elder()
    d =Tools_class.bebek_tools.set_char(5,6,frame,dwarf,pitch_config_dict,board_position_dict,char_dict_pic)
